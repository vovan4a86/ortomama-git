...
function addProductChar(id, e) {
	e.preventDefault();
	let name = $('input[name=new_char_name]');
	let value = $('input[name=new_char_value]');
	let url = '/ajax/add-product-char/';
...
function deleteChar(elem, e) {
	e.preventDefault();

	if(!confirm('Точно удалить эту характеристику?')) return;

	const product = $(elem).closest('.char').data('product');
	const char = $(elem).closest('.char').data('id');
	let url = '/ajax/delete-product-char/';

	sendAjax(url, {char,product}, function (json) {
		if (json.success === true) {
			$(elem).closest('.char').fadeOut(300, function(){ $(this).remove(); });
		}
	})
}
