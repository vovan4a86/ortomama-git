        ...
        @if (count($products))
            <table class="table table-striped table-v-middle">
                ...
                @foreach ($products as $item)
                    @php
                        $item_catalog_ids = $item->catalog()->pluck('catalog_id')->all();
                    @endphp
                        @foreach($item_catalog_ids as $catalog_id)
                            <tr data-id="{{ $item->id }}">
                                <td>
                                    <div class="checkbox">
                                        <label>
                                ...
                                <td><a href="{{ route('admin.catalog.productEdit', [
                                ...
                                <td>{{ $item->article }}</td>
                                <td>
                                    <a href="{{ route('admin.catalog.catalogEdit', $catalog_id) }}">
                                        {{ $item->catalog()->where('catalog_id', $catalog_id)->first()->name }}
                                    </a>
                                </td>
                                <td>{{ number_format($item->price, 0, '.', ' ') }}</td>
                                <td>
                                    <a class="glyphicon glyphicon-trash"
                                       href="{{ route('admin.catalog.productDel', [
                                                        'catalog_id' => $catalog_id,
                                                        'id' => $item->id
                                                    ]) }}"
                                       style="font-size:20px; color:red;" title="Удалить"
                                       onclick="return productDel(this)"></a>
                                </td>
                            </tr>
                        @endforeach
                @endforeach
